...
use kairei::provider::config::{
    ...
    ErrorSeverity, ProviderConfigError, ValidationError,
};
use serde_json::json;
...
fn test_validation_e2e_flow() {
    ...
    assert!(type_check_result.is_ok(), "Type checking should pass for valid config");

    // 2. Evaluation phase
    let evaluator = EvaluatorValidator::default();
    let eval_result = evaluator.validate(&config);
    assert!(eval_result.is_ok(), "Evaluation should pass for valid config");

    // 3. Collecting validation results
    let collector = type_checker.validate_collecting(&config);
    assert!(!collector.has_errors(), "Collector should not have errors for valid config");
    assert_eq!(collector.errors.len(), 0, "Collector should have 0 errors for valid config");
    ...
    assert!(type_check_result.is_ok(), "Type checking should pass for config with semantic errors");

    // 2. Evaluation phase (should fail)
    let eval_result = evaluator.validate_provider_specific(&invalid_config);
    assert!(eval_result.is_err(), "Evaluation should fail for config with invalid ttl");

    // 3. Collecting validation results
    let collector = evaluator.validate_collecting(&invalid_config);
    assert!(collector.has_errors(), "Collector should have errors for invalid config");
    assert!(collector.errors.len() > 0, "Collector should have at least one error for invalid config");
...
fn test_validation_e2e_with_multiple_errors() {
    ...
    assert!(collector.errors.len() >= 1, "Collector should have at least one error for config with invalid values");

    // Verify that at least one error was collected for an invalid field
    let error_fields: Vec<String> = collector.errors.iter()
    ...
    assert!(!error_fields.is_empty(), "Should have at least one error for an invalid field");
}
...
fn test_validation_e2e_with_warnings() {
    ...
    assert!(!collector.has_errors(), "Collector should not have errors for config with potential warnings");
...
fn test_validation_e2e_error_collection_order() {
    ...
    assert!(!collector.errors.is_empty(), "Collector should have at least one error");
    ...
    assert!(!error_fields.is_empty(), "Should have at least one identified error field");
}
