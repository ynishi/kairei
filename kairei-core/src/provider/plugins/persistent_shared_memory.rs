...
impl PersistentSharedMemoryPlugin {
    ...
    fn initialize(&mut self) {
        ...
        self.backend = match self.config.persistence.backend_type {
            crate::provider::config::plugins::BackendType::GCPStorage => {
                if let crate::provider::config::plugins::BackendSpecificConfig::GCP(ref _config) = self.config.persistence.backend_config {
            ...
            crate::provider::config::plugins::BackendType::LocalFileSystem => {
                if let crate::provider::config::plugins::BackendSpecificConfig::Local(ref _config) = self.config.persistence.backend_config {
    ...
    fn start_sync_task(&mut self) {
        ...
        let backend = Arc::new(DummyStorageBackend {}); // This will be replaced with self.backend.clone() when implemented
...
impl SharedMemoryCapability for PersistentSharedMemoryPlugin {
    ...
    async fn list_keys(&self, pattern: &str) -> Result<Vec<String>, SharedMemoryError> {
        ...
        // Compile pattern
        let glob_pattern =
            glob::Pattern::new(pattern).map_err(|e| SharedMemoryError::PatternError(e.to_string()))?;
