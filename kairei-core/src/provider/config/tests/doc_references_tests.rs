...
use crate::provider::config::{
    ProviderConfigError, ProviderError, SchemaError, ValidationError, doc_references,
};
...
fn test_validation_doc_references() {
    ...
    assert!(
        doc_references::validation::invalid_value()
            .contains(doc_references::anchors::ERROR_HANDLING)
    );
    assert!(
        doc_references::validation::constraint_violation()
            .contains(doc_references::anchors::ERROR_HANDLING)
    );
    assert!(
        doc_references::validation::dependency_error()
            .contains(doc_references::anchors::ERROR_HANDLING)
    );
}
...
fn test_provider_doc_references() {
    ...
    assert!(
        doc_references::provider::initialization()
    ...
    assert!(
        doc_references::provider::configuration()
            .contains(doc_references::anchors::TROUBLESHOOTING)
    );
}
