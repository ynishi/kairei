...
pub trait RelevantMemoryCapability: ProviderPlugin + Send + Sync {
    ...
    async fn store_memory_item(&self, item: DetailedMemoryItem) -> Result<MemoryId, SistenceMemoryError>;

    /// Retrieve a specific memory item by ID with full metadata
    async fn retrieve_memory_item(&self, id: &MemoryId) -> Result<DetailedMemoryItem, SistenceMemoryError>;
    
    /// Update an existing memory item preserving all metadata
    async fn update_memory_item(&self, item: DetailedMemoryItem) -> Result<(), SistenceMemoryError>;
...
pub trait DetailedImportanceEvaluator: Send + Sync {
    ...
    fn evaluate_first_impression(
        &self, 
        item: &DetailedMemoryItem
    ) -> f32;
    
    /// Evaluate network centrality
    fn evaluate_network_centrality(
        &self, 
        item_id: &MemoryId
    ) -> f32;
