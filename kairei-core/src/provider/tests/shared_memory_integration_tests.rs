...
use crate::config::{PluginConfig, ProviderConfig, ProviderConfigs, ProviderSecret, SecretConfig};
...
use crate::provider::event_bus::EventBus;
...
fn create_provider_config(name: &str, namespace: &str) -> ProviderConfig {
    ...
    ProviderConfig {
        name: name.to_string(),
        provider_type: crate::provider::provider::ProviderType::OpenAIChat,
        plugin_configs,
        ..Default::default()
    }
}
...
impl Provider for MockProvider {
    ...
    ) -> ProviderResult<crate::provider::request::ProviderResponse> {
        Ok(crate::provider::request::ProviderResponse::default())
    }
    
    async fn capabilities(&self) -> crate::provider::capability::Capabilities {
        crate::provider::capability::Capabilities::default()
    }
...
async fn test_provider_registry_integration() -> ProviderResult<()> {
    ...
    // Register provider
    registry.register_provider_with("test_provider", &provider_config, &secret, provider).await?;
    ...
    assert!(namespaces.is_empty(), "Namespaces should be empty after shutdown");
...
async fn test_plugin_registration_and_cleanup() -> ProviderResult<()> {
    ...
    for i in 1..=5 {
        ...
        registry
            .register_provider_with(&format!("provider{}", i), &provider_config, &secret, provider)
            .await?;
    ...
    for i in 1..=5 {
        ...
        assert!(!Arc::ptr_eq(&plugin, &Arc::new(())), "Plugin for {} should exist", namespace);
    }
    ...
    let namespaces = registry.list_shared_memory_namespaces();
    assert!(namespaces.is_empty(), "All namespaces should be cleaned up after shutdown");
