    ...
    fn provide_documented_parsers(&self) -> Vec<Box<dyn DocParserExt<Token, Box<dyn Any>>>> {
        ...
        for category in &self.categories {
            for i in 0..self.parsers_per_category {
                let parser_name =
                    format!("parse_{}_{}", category.to_string().to_lowercase(), i + 1);
                ...
                impl<I, O> kairei_core::analyzer::core::Parser<I, O> for DummyParser {
                    fn parse(
                        ...
                        _input: &[I],
                        _pos: usize,
                    ) -> kairei_core::analyzer::core::ParseResult<O> {
                ...
                let boxed: Box<dyn DocParserExt<Token, Box<dyn Any>>> = unsafe {
                    std::mem::transmute(Box::new(parser) as Box<dyn DocParserExt<(), ()>>)
                };
...
fn test_documentation_system_integration() {
    ...
    collector.register_provider(Box::new(TestParserDocumentationProvider::new(
        categories.clone(),
        3,
    )));
    ...
    collector.register_provider(Box::new(TestParserDocumentationProvider::new(
        vec![ParserCategory::Expression],
        2,
    )));

    collector.register_provider(Box::new(TestParserDocumentationProvider::new(
        vec![ParserCategory::Statement],
        2,
    )));
    ...
    assert_eq!(
        collection
            .get_by_category(&ParserCategory::Expression)
            .len(),
        2
    );
    assert_eq!(
        collection.get_by_category(&ParserCategory::Statement).len(),
        2
    );
}
...
fn test_documentation_system_with_related_parsers() {
    ...
    impl kairei_core::analyzer::DocumentationProvider for RelatedParsersProvider {
        fn provide_documented_parsers(&self) -> Vec<Box<dyn DocParserExt<Token, Box<dyn Any>>>> {
            ...
            let binary_expr_doc =
                DocBuilder::new("parse_binary_expression", ParserCategory::Expression)
            ...
            let primary_expr_doc =
                DocBuilder::new("parse_primary_expression", ParserCategory::Expression)
            ...
            let binary_expr_parser = DummyParser {
                doc: binary_expr_doc,
            };
            let primary_expr_parser = DummyParser {
                doc: primary_expr_doc,
            };

            // Box them with the right type
            unsafe {
                result.push(std::mem::transmute(
                    Box::new(expr_parser) as Box<dyn DocParserExt<(), ()>>
                ));
                result.push(std::mem::transmute(
                    Box::new(binary_expr_parser) as Box<dyn DocParserExt<(), ()>>
                ));
                result.push(std::mem::transmute(
                    Box::new(primary_expr_parser) as Box<dyn DocParserExt<(), ()>>
                ));
            }
...
fn test_documentation_validation() {
    ...
    impl kairei_core::analyzer::DocumentationProvider for InvalidDocumentationProvider {
        fn provide_documented_parsers(&self) -> Vec<Box<dyn DocParserExt<Token, Box<dyn Any>>>> {
            ...
            let mut missing_desc_doc =
                DocBuilder::new("missing_description", ParserCategory::Expression)
                    .example("example")
                    .build();
            missing_desc_doc.description = "".to_string();

            // 2. Missing examples
            let missing_examples_doc =
                DocBuilder::new("missing_examples", ParserCategory::Expression)
            ...
            let missing_desc_parser = DummyParser {
                doc: missing_desc_doc,
            };
            let missing_examples_parser = DummyParser {
                doc: missing_examples_doc,
            };
            let bad_reference_parser = DummyParser {
                doc: bad_reference_doc,
            };

            // Box them with the right type
            unsafe {
                result.push(std::mem::transmute(
                    Box::new(missing_desc_parser) as Box<dyn DocParserExt<(), ()>>
                ));
                result.push(std::mem::transmute(
                    Box::new(missing_examples_parser) as Box<dyn DocParserExt<(), ()>>
                ));
                result.push(std::mem::transmute(
                    Box::new(bad_reference_parser) as Box<dyn DocParserExt<(), ()>>
                ));
            }
    ...
    assert!(
        issues
            .iter()
            .any(|issue| issue.contains("missing_description")
                && issue.contains("empty description"))
    );
    assert!(
        issues
            .iter()
            .any(|issue| issue.contains("missing_examples") && issue.contains("no examples"))
    );
    assert!(
        issues.iter().any(|issue| issue.contains("bad_reference")
            && issue.contains("non-existent related parser"))
    );
}
