...
use crate::provider::config::{
    validators::{TypeCheckerValidator, EvaluatorValidator},
    validator::{ProviderConfigValidator, CollectingValidator},
};
...
fn provider_config_strategy() -> impl Strategy<Value = HashMap<String, Value>> {
    ...
    (provider_types, ttl_values, chunk_size_values, max_tokens_values)
        .prop_map(|(provider_type, ttl, chunk_size, max_tokens)| {
            ...
            let capabilities_map = serde_json::Map::from_iter(
                capabilities.into_iter().map(|(k, v)| (k, v))
            );
...
fn provider_config_with_missing_fields_strategy() -> impl Strategy<Value = HashMap<String, Value>> {
    ...
    (provider_types, include_type, include_ttl, include_chunk_size, include_max_tokens, include_capabilities)
        .prop_map(|(provider_type, include_type, include_ttl, include_chunk_size, include_max_tokens, include_capabilities)| {
            let mut config = HashMap::new();
