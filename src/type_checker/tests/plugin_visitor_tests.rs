...
fn test_plugin_request_validation() {
    ...
    assert!(visitor.validate_plugin_request(&valid_request, &MockPlugin{}, &mut ctx).is_ok());
    ...
    assert!(visitor.validate_plugin_request(&invalid_request, &MockPlugin{}, &mut ctx).is_err());
}
...
fn test_plugin_response_validation() {
    ...
    assert!(visitor.validate_plugin_response(&response, &mut ctx).is_ok());
}
...
fn test_value_type_validation() {
    ...
    // Test valid types
    assert!(visitor.validate_value_type(&Value::String("test".to_string())).is_ok());
    ...
    assert!(visitor.validate_value_type(&Value::List(vec![
