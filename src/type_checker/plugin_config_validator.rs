use crate::{
    ...
    provider::config::{
        TypeCheckerValidator, ProviderConfigValidator,
        ErrorCollector, CollectingValidator,
    },
                ...
                let config_map: HashMap<String, JsonValue> = config
                    ...
                    .map(|(k, v)| match v {
                        ...
                        ast::Literal::Number(n) => {
                            if n.contains('.') {
                                (k.clone(), JsonValue::Number(serde_json::Number::from_f64(n.parse().unwrap_or(0.0)).unwrap()))
                            } else {
                                (k.clone(), JsonValue::Number(serde_json::Number::from(n.parse::<i64>().unwrap_or(0))))
                            }
                        },
